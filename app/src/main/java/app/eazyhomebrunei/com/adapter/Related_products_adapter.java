...
public class Related_products_adapter  extends RecyclerView.Adapter<Related_products_adapter.viewHolder> {
    private List<related_pro> fdproductlist;

    private Context mcontext;
    String imageurl;
    String Token;
    int wishId;
    private Related_products_adapter.CartItemClickListener mListener;

    public interface CartItemClickListener{
        void CartItemClick(int position);
    }
    public void setOnItemClickListener(Related_products_adapter.CartItemClickListener listener){
        mListener =listener;
    }
    public Related_products_adapter(Context mcontext,List<related_pro> listitem) {
    ...
    @NonNull
    @Override
    public Related_products_adapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.related_products_adapter,parent,false);
        mcontext=parent.getContext();
        // Toast.makeText(v.getContext(), "imagee"+imageurl, Toast.LENGTH_SHORT).show();

        return  new Related_products_adapter.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final Related_products_adapter.viewHolder holder, final int position) {
        ...
        final related_pro product=fdproductlist.get(position);
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if (b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(Related_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    Related_products_adapter.this.addToWishList(product.getId(), position);
                    return;
                }
                holder.tb_wishlist.setChecked(false);
                holder.tb_wishlist.setBackgroundDrawable(Related_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                Related_products_adapter.this.remove_wishlist(product.getWishId(), position);
            }
        });
...
//                intent.putExtra("detailed_description",fdproductlist.get(position).getDetailed_description());
            ...
            holder.add_cart.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        Related_products_adapter.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    Related_products_adapter.this.addToCart(1, product.getId(), holder, position);
    ...
    private void addToCart(int quantity, final int pid, final Related_products_adapter.viewHolder holder, final int position) {
