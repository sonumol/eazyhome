...
public class all_products_adapter extends RecyclerView.Adapter<all_products_adapter.viewHolder> {
    private List<all_products> fdproductlist;

    private Context mcontext;
    String imageurl;
    String Token;
    int wishId;
    private all_products_adapter.CartItemClickListener mListener;

    public interface CartItemClickListener{
        void CartItemClick(int position);
    }
    public void setOnItemClickListener(all_products_adapter.CartItemClickListener listener){
        mListener =listener;
    }
    public all_products_adapter(Context mcontext,List<all_products> listitem) {
    ...
    @NonNull
    @Override
    public all_products_adapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.all_products_adapter,parent,false);
        mcontext=parent.getContext();
        // Toast.makeText(v.getContext(), "imagee"+imageurl, Toast.LENGTH_SHORT).show();

        return  new all_products_adapter.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final all_products_adapter.viewHolder holder, final int position) {
        final all_products product=fdproductlist.get(position);
...
//        String offerprice=product.getOffer_price();
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if (b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(all_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    all_products_adapter.this.addToWishList(product.getId(), position);
                    return;
                }
                holder.tb_wishlist.setChecked(false);
                holder.tb_wishlist.setBackgroundDrawable(all_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                all_products_adapter.this.remove_wishlist(product.getWishid(), position);
            }
        });
            ...
            holder.add_cart.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        all_products_adapter.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    all_products_adapter.this.addToCart(1, product.getId(), holder, position);
...
//                    lnr1.setVisibility(View.VISIBLE);
    ...
    private void addToCart(int quantity, final int pid, final all_products_adapter.viewHolder holder, final int position) {
