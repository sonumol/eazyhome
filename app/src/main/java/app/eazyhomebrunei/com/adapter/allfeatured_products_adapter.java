...
public class allfeatured_products_adapter extends RecyclerView.Adapter<allfeatured_products_adapter.viewHolder> {
    private List<all_featured_pro> fdproductlist;
    ...
    private allfeatured_products_adapter.CartItemClickListener mListener;

    public interface CartItemClickListener{
        void CartItemClick(int position);
    }
    public void setOnItemClickListener(allfeatured_products_adapter.CartItemClickListener listener){
        mListener =listener;
    }


    public allfeatured_products_adapter(Context mcontext,List<all_featured_pro> listitem) {
    ...
    @NonNull
    @Override
    public allfeatured_products_adapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.all_featured_products,parent,false);
        mcontext=parent.getContext();
        // Toast.makeText(v.getContext(), "imagee"+imageurl, Toast.LENGTH_SHORT).show();

        return  new allfeatured_products_adapter.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final allfeatured_products_adapter.viewHolder holder, final int position) {
        ...
        final all_featured_pro product=fdproductlist.get(position);
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
              //  Toast.makeText(mcontext,"flag"+product.getFlag(),Toast.LENGTH_LONG).show();
                ...
                if (b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(allfeatured_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    allfeatured_products_adapter.this.addToWishList(product.getId(), position);
                    return;
                }
                holder.tb_wishlist.setChecked(false);
                holder.tb_wishlist.setBackgroundDrawable(allfeatured_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                allfeatured_products_adapter.this.remove_wishlist(product.getWishId(), position);
            }
        });
...
//                intent.putExtra("detailed_description",fdproductlist.get(position).getDetailed_description());
            ...
            holder.add_cart.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        allfeatured_products_adapter.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    allfeatured_products_adapter.this.addToCart(1, product.getId(), holder, position);
    ...
    private void addToCart(int quantity, final int pid, final allfeatured_products_adapter.viewHolder holder, final int position) {
