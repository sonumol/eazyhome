...
public class offer_products_adapter extends RecyclerView.Adapter<offer_products_adapter.viewHolder> {
    private List<offer_product> offproductlist;
    private offer_products_adapter.CartItemClickListener mListener;
    ...
    public void setOnItemClickListener(offer_products_adapter.CartItemClickListener listener){
        mListener =listener;
    }
    public offer_products_adapter( Context mcontext,List<offer_product> listitem) {
    ...
    @NonNull
    @Override
    public offer_products_adapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.offer_products_adapter,parent,false);
        mcontext=parent.getContext();
        return  new offer_products_adapter.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final offer_products_adapter.viewHolder holder, final int position) {
        final offer_product product=offproductlist.get(position);
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
               // Toast.makeText(mcontext,"flag"+product.getFlag(),Toast.LENGTH_LONG).show();
                ...
                if (b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(offer_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    offer_products_adapter.this.addToWishList(product.getId(), position);
                    return;
                }
                holder.tb_wishlist.setChecked(false);
                holder.tb_wishlist.setBackgroundDrawable(offer_products_adapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                offer_products_adapter.this.remove_wishlist(product.getWishid(), position);
            }
        });
                ...
                public void onClick(View view) {
                   // Toast.makeText(mcontext,"ADD TO CART",Toast.LENGTH_LONG).show();
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        offer_products_adapter.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    offer_products_adapter.this.addToCart(1, product.getId(), holder, position);
...
//                // map.put("product_id", modelList.get(position).getProduct_id());
              ...
              //  intent.putExtra("detailed_description",offproductlist.get(position).getDetailed_description());
                intent.putExtra("status",offproductlist.get(position).getFlag());
...
//                    lnr1.setVisibility(View.VISIBLE);
    ...
    private void addToCart(int quantity, final int pid, final offer_products_adapter.viewHolder holder, final int position) {
