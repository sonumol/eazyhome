...
import app.eazyhomebrunei.com.model.catgeory;
import app.eazyhomebrunei.com.view_my_cart;

import static app.eazyhomebrunei.com.Config.BaseURL.PATH;

public class vieallsubincategoryadapter extends RecyclerView.Adapter<vieallsubincategoryadapter.viewHolder> {
    private List<catgeory> latest_productlist;

    private Context mcontext;
    String Token;
    int pid;
    int wishId;
    private vieallsubincategoryadapter.CartItemClickListener mListener;

    public interface CartItemClickListener{
        void CartItemClick(int position);
    }
    public void setOnItemClickListener(vieallsubincategoryadapter.CartItemClickListener listener){
        mListener =listener;
    }
    public vieallsubincategoryadapter( Context mcontext,List<catgeory> listitem) {
    ...
    @NonNull
    @Override
    public vieallsubincategoryadapter.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.categoryproductadapter,parent,false);
        mcontext=parent.getContext();

        return  new vieallsubincategoryadapter.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final vieallsubincategoryadapter.viewHolder holder, final int position) {
        final catgeory product=latest_productlist.get(position);
...
//        holder.offer_price.setText(offerprice);
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if(b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(vieallsubincategoryadapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    vieallsubincategoryadapter.this.addToWishList(product.getPid(), position);
                    return;
                }
                else
                {
                    holder.tb_wishlist.setChecked(false);
                    holder.tb_wishlist.setBackgroundDrawable(vieallsubincategoryadapter.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                    vieallsubincategoryadapter.this.remove_wishlist(product.getWishId(), position);
            ...
            holder.add_cart.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        vieallsubincategoryadapter.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    vieallsubincategoryadapter.this.addToCart(1, product.getPid(), holder, position);
...
//                intent.putExtra("detailed_description",latest_productlist.get(position).getDetailed_description());
    ...
    private void addToCart(int quantity, final int pid, final vieallsubincategoryadapter.viewHolder holder, final int position) {
...
//                    lnr1.setVisibility(View.VISIBLE);
    ...
    private void addToWishList(final int pid, final int position) {
        ...
        String URL_ADD_TO_WISHLIST=PATH+"AddToWishlist?pid="+pid;
        //  Toast.makeText(mcontext,"res"+lid,Toast.LENGTH_LONG).show();
