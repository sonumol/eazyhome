...
public class view_all_latest_products  extends RecyclerView.Adapter<view_all_latest_products.viewHolder> {
    ...
    int pid;
    int wishId;
    private view_all_latest_products.CartItemClickListener mListener;

    public interface CartItemClickListener{
        void CartItemClick(int position);
    }
    public void setOnItemClickListener(view_all_latest_products.CartItemClickListener listener){
        mListener =listener;
    }

    public view_all_latest_products( Context mcontext,List<latest_products> listitem) {
    ...
    @NonNull
    @Override
    public view_all_latest_products.viewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View v= LayoutInflater.from(parent.getContext()).inflate(R.layout.view_latest_products_adapter,parent,false);
        mcontext=parent.getContext();

        return  new view_all_latest_products.viewHolder(v);
    }

    @Override
    public void onBindViewHolder(@NonNull final view_all_latest_products.viewHolder holder, final int position) {
        ...
        holder.tb_wishlist.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            public void onCheckedChanged(CompoundButton compoundButton, boolean b) {
                ...
                if (b) {
                    holder.tb_wishlist.setChecked(true);
                    holder.tb_wishlist.setBackgroundDrawable(view_all_latest_products.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_black));
                    view_all_latest_products.this.addToWishList(product.getId(), position);
                    return;
                }
                holder.tb_wishlist.setChecked(false);
                holder.tb_wishlist.setBackgroundDrawable(view_all_latest_products.this.mcontext.getResources().getDrawable(R.drawable.ic_favorite_border_black));
                view_all_latest_products.this.remove_wishlist(product.getWishid(), position);
            }
        });
            ...
            holder.add_cart.setOnClickListener(new View.OnClickListener() {
                public void onClick(View view) {
                    ...
                    if (holder.add_cart.getText().toString().matches("IN CART")) {
                        view_all_latest_products.this.showDialog();
                    } else {
                        ...
                        new Thread(new Runnable() {
                            public void run() {
                                try {
                                    Thread.sleep(1000);
                                    view_all_latest_products.this.addToCart(1, product.getId(), position);
